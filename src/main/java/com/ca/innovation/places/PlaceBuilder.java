package com.ca.innovation.places;

import com.ca.innovation.places.exception.GooglePlacesException;
...
public class PlaceBuilder {
    ...
    public JSONObject toJson() {
        JSONObject obj = new JSONObject().put(GooglePlacesInterface.OBJECT_LOCATION, new JSONObject().put("lat", lat).put("lng", lng))
                .put(GooglePlacesInterface.STRING_NAME, name).put(GooglePlacesInterface.STRING_TYPES, new JSONArray(types));
        if (accuracy != -1)
            obj.put(GooglePlacesInterface.INTEGER_ACCURACY, accuracy);
        if (phoneNumber != null)
            obj.put(GooglePlacesInterface.STRING_PHONE_NUMBER, phoneNumber);
        if (address != null)
            obj.put(GooglePlacesInterface.STRING_ADDRESS, address);
        if (website != null)
            obj.put(GooglePlacesInterface.STRING_WEBSITE, website);
        if (locale != null)
            obj.put(GooglePlacesInterface.STRING_LANGUAGE, locale.getLanguage());
        return obj;
    }
}
